...
interface MeetingData {
  ...
  transcription?: {
    transcript?: string
    summary?: string
    notes?: Record<string, string> // Dynamic notes with sub-headers
    highlights?: string[] // Deprecated, use notes instead
    conclusion?: string
    segments?: Segment[]
    language?: string
  }
  actionItems?: Array<{ text: string; assignee?: string; dueDate?: Date; completed?: boolean }>
  participants?: number
  videoUrl?: string
  audioUrl?: string
  tags?: string[]
...
export default function MeetingDetailPage() {
  ...
  const fetchMeeting = useCallback(async () => {
    ...
    try {
      setIsLoading(true)
      const response = await api.getMeeting(meetingId) as { success: boolean; meeting: MeetingData; fileUrl?: string }
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (analyticsData) {
        ...
        // Update action items from analytics if available
        if (analyticsData.actionItems && analyticsData.actionItems.length > 0) {
          setMeeting(prev => prev ? { ...prev, actionItems: analyticsData.actionItems } : null)
        }
      }
    } catch (error: any) {
  ...
  const handleUpdateContent = async (field: 'executiveSummary' | 'highlights' | 'notes' | 'conclusion' | 'actionItems', value: any) => {
    ...
    try {
      ...
      if (field === 'executiveSummary') {
        ...
        updateData.actionItems = value.map((text: string) => ({ text, completed: false }))
      }
  ...
  const notes = meeting.transcription?.notes || {}
            ...
            <div>
                      ...
                      return (
                        <div key={index} className="flex items-start gap-2">
                          ...
                          <div className="text-sm leading-tight flex-1">
                            {item.text}
                            {item.assignee && (
                              <span className="text-xs text-muted-foreground block mt-0.5">Assignee: {item.assignee}</span>
                            )}
                          </div>
                        </div>
                      )
              ...
              {tasks.length > 0 && (
                <div className="flex items-center justify-between mb-4 bg-purple-50 rounded-lg p-2">
                  ...
                  <Button size="sm" variant="outline" className="h-6 text-xs px-2 rounded-full" onClick={() => router.push('/dashboard/kanban')}>
                    Buat
          ...
          <div className="px-6 py-4 space-y-6 pb-24">
            ...
            <div>
                ...
                <Dialog open={editingField === 'executiveSummary'} onOpenChange={(open) => {
                  ...
                  <DialogContent>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setEditValues({ ...editValues, executiveSummary: e.target.value })}
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                {executiveSummary || "Tidak ada summary tersedia"}
              </p>
            </div>

            {/* Notes - Dynamic sub-headers */}
            {Object.keys(notes).length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-base font-semibold text-purple-700">Catatan Rapat</h2>
                  <Dialog open={editingField === 'notes'} onOpenChange={(open) => {
                    if (!open) setEditingField(null)
                    else setEditingField('notes')
                  }}>
                    <DialogTrigger asChild>
                      <button className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                        <IconPencil className="h-3.5 w-3.5" />
                    ...
                    <DialogContent className="max-w-2xl">
                      ...
                      <Textarea
                        defaultValue={Object.entries(notes).map(([key, value]) => `${key}:\n${value}`).join('\n\n')}
                        onChange={(e) => {
                          ...
                          lines.forEach(line => {
                            if (line.trim().endsWith(':')) {
                              if (currentKey) {
                                notesObj[currentKey] = currentValue.join('\n').trim()
                              }
                          ...
                          setEditValues({ ...editValues, notes: notesObj })
                        }}
                      ...
                      <div className="flex justify-end gap-2">
                        <Button variant="outline" onClick={() => setEditingField(null)}>Batal</Button>
                        <Button onClick={() => handleUpdateContent('notes', editValues.notes || notes)}>Simpan</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                </div>
                ...
                <div className="space-y-6">
                  {Object.entries(notes).map(([header, content]) => (
                    <div key={header}>
            ...
            {/* Fallback to highlights if notes not available */}
            {Object.keys(notes).length === 0 && highlights.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Dialog open={editingField === 'highlights'} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>Edit Discussion Highlights</DialogTitle>
                      </DialogHeader>
                      <Textarea
                        defaultValue={highlights.join('\n')}
                        onChange={(e) => setEditValues({ ...editValues, highlights: e.target.value.split('\n').filter(h => h.trim()) })}
                        className="min-h-[200px]"
                        placeholder="Masukkan highlights, satu per baris"
                      />
                      <div className="flex justify-end gap-2">
                        <Button variant="outline" onClick={() => setEditingField(null)}>Batal</Button>
                        <Button onClick={() => handleUpdateContent('highlights', editValues.highlights || highlights)}>Simpan</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Dialog open={editingField === 'actionItems'} onOpenChange={(open) => {
                  if (!open) setEditingField(null)
                  else setEditingField('actionItems')
                }}>
                  <DialogTrigger asChild>
                    <button className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
                      <IconPencil className="h-3.5 w-3.5" />
                  ...
                  <DialogContent>
                    ...
                    <Textarea
                      defaultValue={actionItems.map(item => item.text).join('\n')}
                      onChange={(e) => setEditValues({ ...editValues, actionItems: e.target.value.split('\n').filter(a => a.trim()) })}
                      className="min-h-[200px]"
                      placeholder="Masukkan action items, satu per baris"
                    />
                    <div className="flex justify-end gap-2">
                      <Button variant="outline" onClick={() => setEditingField(null)}>Batal</Button>
                      <Button onClick={() => handleUpdateContent('actionItems', editValues.actionItems || actionItems.map(a => a.text))}>Simpan</Button>
                    </div>
                  </DialogContent>
                </Dialog>
              </div>
            ...
            <div>
                ...
                <Dialog open={editingField === 'conclusion'} onOpenChange={(open) => {
                  ...
                  <DialogContent>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setEditValues({ ...editValues, conclusion: e.target.value })}
                      className="min-h-[200px]"
                    />
                    <div className="flex justify-end gap-2">
                      <Button variant="outline" onClick={() => setEditingField(null)}>Batal</Button>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                {conclusion || "Tidak ada conclusion tersedia"}
              </p>
            </div>
          </div>
        </div>
