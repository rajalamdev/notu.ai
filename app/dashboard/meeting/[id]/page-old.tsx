...
export default function MeetingDetailPage() {
  ...
  const fetchMeeting = useCallback(async () => {
    ...
    try {
      setIsLoading(true)
      const response = await api.getMeeting(meetingId) as { meeting: MeetingData }
  ...
  // Handle export
  const handleExport = async (format: 'json' | 'txt' | 'srt' | 'vtt') => {
    if (!isReady || !meetingId) return
    ...
    try {
      ...
      toast.error("Gagal mengexport transcript")
    }
  }
  ...
  // Use meeting data or fallback to demo data
  const meetingTitle = meeting.title || "Rapat HMIF Periode 2025/2026"
  const meetingSummary = meeting.summary || "Pada rapat ini membahas tentang pengkajian ad/art periode himpunan 2025/2026, banyak sekali perubahan ad/art karena sudah tidak relevannya dengan zamannya. Topik utama meliputi evaluasi struktur organisasi, perencanaan kegiatan semester depan, dan pembaharuan anggaran operasional."
  const meetingHighlights = meeting.highlights || [
    "• Perubahan AD/ART periode 2025/2026 disetujui dengan beberapa revisi minor",
    "• Struktur organisasi baru akan mulai berlaku pada semester genap",
    "• Anggaran operasional meningkat 15% dari periode sebelumnya",
    "• Kegiatan unggulan tahun ini adalah workshop teknologi dan seminar nasional"
  ]
  const meetingActionItems = meeting.actionItems || [
    { title: "Revisi dokumen AD/ART dan distribusi ke anggota", assignee: "Sekretaris", completed: false },
    { title: "Persiapan rapat koordinasi dengan pihak fakultas", assignee: "Ketua", completed: false },
    { title: "Finalisasi anggaran kegiatan semester genap", assignee: "Bendahara", completed: false }
  ]
  const meetingConclusion = meeting.conclusion || "Rapat berjalan dengan lancar dan produktif. Semua agenda telah dibahas dengan tuntas. Hasil keputusan akan didokumentasikan dan didistribusikan ke seluruh anggota dalam waktu 3 hari kerja."
  const meetingParticipants = meeting.participants || [
    { name: "Raj", speakingTime: 45 },
    { name: "Sarah", speakingTime: 30 },
    { name: "Ahmad", speakingTime: 15 },
    { name: "Lainnya", speakingTime: 10 }
  ]
  const meetingTopics = meeting.topics || [
    { title: "Pengkajian AD/ART", percentage: 35 },
    { name: "Struktur Organisasi", percentage: 25 },
    { name: "Anggaran", percentage: 20 },
    { name: "Kegiatan", percentage: 15 },
    { name: "Lain-lain", percentage: 5 }
  ]
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Link Button */}
          <Button variant="outline" className="rounded-full h-9 px-4 flex items-center gap-2" onClick={() => {
          ...
          {/* Download */}
          <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => handleExport('txt')}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
