...
class ApiClient {
  ...
  async getKanbanTasks(token: string): Promise<KanbanResponse> {
    return this.request<KanbanResponse>('/api/tasks/kanban', { token });
  }
  ...
  async createTask(token: string, data: { title: string; description?: string; status?: string; priority?: string; dueDate?: string; assignee?: string; tags?: string[]; meetingId?: string }) {
